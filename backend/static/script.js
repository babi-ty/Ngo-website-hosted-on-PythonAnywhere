...
// Read the mouth shapes ONCE and store them.
var smallPathD = mouthSmallBG.getAttribute('d');
var mediumPathD = mouthMediumBG.getAttribute('d');
var largePathD = mouthLargeBG.getAttribute('d');
// ----------------------------
	...
	if (curEmailIndex > 0) {
		if (value.includes("@")) {
			...
			if (mouthStatus !== "large") {
				...
				// 1. Fade out the mouth (FASTER)
				TweenMax.to([mouthBG, mouthOutline, mouthMaskPath], 0.01, { // 0.2s out
					opacity: 0,
					ease: Expo.easeOut,
					onComplete: function() {
						// 2. Set new shape while invisible
						mouthBG.setAttribute('d', largePathD);
						mouthOutline.setAttribute('d', largePathD);
						mouthMaskPath.setAttribute('d', largePathD);
						
						// 3. Fade back in
						TweenMax.to([mouthBG, mouthOutline, mouthMaskPath], 0.08, { // 0.5s in
							opacity: 1,
							ease: Expo.easeIn
						});

						// --- ANIMATE OTHER PARTS AT THE SAME TIME ---
						TweenMax.to(tooth, 0.2, {x: 3, y: -2, ease: Expo.easeOut});
						TweenMax.to(tongue, 0.8, {y: 2, ease: Expo.easeOut});
						TweenMax.to([eyeL, eyeR], 0.8, {scaleX: .65, scaleY: .65, ease: Expo.easeOut, transformOrigin: "center center"});
					}
				});
			}
		} else {
			...
			if (mouthStatus !== "medium") {
				...
				// 1. Fade out the mouth
				TweenMax.to([mouthBG, mouthOutline, mouthMaskPath], 0.08, {
					opacity: 0,
					ease: Expo.easeOut,
					onComplete: function() {
						// 2. Set new shape while invisible
						mouthBG.setAttribute('d', mediumPathD);
						mouthOutline.setAttribute('d', mediumPathD);
						mouthMaskPath.setAttribute('d', mediumPathD);

						// 3. Fade back in
						TweenMax.to([mouthBG, mouthOutline, mouthMaskPath], 0.1, {
							opacity: 1,
							ease: Expo.easeIn
						});

						// --- ANIMATE OTHER PARTS AT THE SAME TIME ---
						TweenMax.to(tooth, 0.8, {x: 0, y: 0, ease: Expo.easeOut});
						TweenMax.to(tongue, 0.8, {x: 0, y: 1, ease: Expo.easeOut});
						TweenMax.to([eyeL, eyeR], 0.8, {scaleX: .85, scaleY: .85, ease: Expo.easeOut});
					}
				});
			}
		}
		...
		if (mouthStatus !== "small") {
			...
			TweenMax.to([mouthBG, mouthOutline, mouthMaskPath], 0.08, {
				...
				onComplete: function() {
					// 2. Set new shape while invisible
					mouthBG.setAttribute('d', smallPathD);
					mouthOutline.setAttribute('d', smallPathD);
					mouthMaskPath.setAttribute('d', smallPathD);
					...
					// --- ANIMATE OTHER PARTS AT THE SAME TIME ---
					TweenMax.to(tooth, 0.8, {x: 0, y: 0, ease: Expo.easeOut});
					TweenMax.to(tongue, 0.8, {y: 0, ease: Expo.easeOut});
					TweenMax.to([eyeL, eyeR], 0.8, {scaleX: 1, scaleY: 1, ease: Expo.easeOut});
				}
			});
		}
	}
